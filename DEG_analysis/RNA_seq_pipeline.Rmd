...
colnames(pData) <- stri_replace_all_regex(colnames(data$meta_data),
                                  pattern=c('_x', 'x_',
                                            '_e2_c3', 'e3_c3', 'e5_c5'),
                                  replacement=c(''),
                                  vectorize=FALSE)
...
#Also need to subset expression data etc to be the same dim
#Filter remaining dataframes to keep equal matrix dimensions
#data$counts <- data$counts[,colnames(data$counts) %in% data$meta_data$row_names] 
#data$samples <- data$samples[rownames(data$samples) %in% data$meta_data$row_names,]

#length(data$samples$norm.factors)

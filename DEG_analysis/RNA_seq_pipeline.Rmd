...
p <- -log10(0.05)
raw_p <- 0.05
#calculate MT threshold (BH method) # FDR < 0.05
#Convert 0.05 adj-p back into an unadjusted p-value, then -log so we can get values for plot
#Rearrange BH formula to find p or subset results for ad.jp = 0.05 and use it's equivalent p value as the estimate and log it, my maths is crap so subsetting table for now
values <- seq(0.050,0.051, by=0.00001)
deg$adj.P.Val <- round(deg$adj.P.Val, 5) #This is such a janky way I apologise
  adj.p <- deg[(deg$adj.P.Val %in% (values)),c("P.Value")] 
  adj.p <-  min(adj.p)
...
#Todo check labels in plot_vol code
...
1)  To pre-process RNAseq data for typhoid challenge and vaccination studies
...
Source Materials: - <https://ucdavis-bioinformatics-training.github.io/2018-June-RNA-Seq-Workshop/thursday/DE.html> - <https://www.bioconductor.org/packages/devel/workflows/vignettes/RNAseq123/inst/doc/limmaWorkflow.html> - <https://www.youtube.com/watch?v=z36fu178jIQ&ab_channel=LiquidBrainBioinformatics>
...
samples <- data$samples[(data$meta_data$time >=0),] #subset samples by metadata rows that are post-chall
...
If a sample is TD, a = 1, if it is not a TD sample, a = 0.\
...
#Model design matrix
design <- model.matrix(~0+ time + study_arm + sex + sequence_pool, data = data$samples)
...
rankifremoved <- sapply(1:ncol(design), function (x) qr(design[,-x])$rank) #function calculates the rank of the matrix - each column
...
#results <- contrast_2_lm(group = c("V7", "V1", "D0"), control = c("V0"),efit=fit, des_mat = design)
results <- contrast_2_lm(group = groups, control = c("time0"),efit=fit, des_mat = design) #all times
...
ns <- plot_vol(deg, lab = 'ns',genes = genes_fc, title = plot_title)
...
tableexpsign <- model.matrix(~Time + Array_experiment + age + Sex + StudyArm, data = tableexpsign_12h)
...
vax_time <- paste0(samples$study_arm, samples$time_point3) #compare D0_TCV  D0_ViPS
vax_diag <- paste0(samples$study_arm, samples$diagnosis) #compare D0_TCV  D0_ViPS
...
design <- model.matrix(~0+ time_diag + study_arm + diagnosis + sex + sequence_pool, data = data$samples)
...
design <- design[,-c(14,17)] #-control  #-dag_TD                      
...
results <- contrast_2_lm(group = c("V1TD", "V1nTD"), efit=fit, des_mat = design)
...
#need limma for all the modelling thoug

...
#Check for Nas
sapply(data$meta_data, function(x) sum(is.na(x)))
test <- data$meta_data[sapply(data$meta_data, function(x) sum(is.na(x))),]
...
#Model design matrix
design <- model.matrix(~0+ time_point3 + sex + study_arm + sequence_pool, data = data$meta_data) #514 samples NA issue, can subset exprs data by design rows if we want to use this
#Leave out diagnosis for now + diagnosis

#Check linear dependencies 
#My bet is on time_point and study arm, as the control group wont have vacc time points, therefore should nest these factors?
ncol(design) #17
qr(design)$rank #16 - unequal rank = linear dependencies
...
which(rankifremoved == max(rankifremoved)) #1:12 exc col 9 - time/study arm

#Manual nested matrix
length(unique(data$meta_data$lab_id)) #105
design2 <- model.matrix(~0+lab_id, data = data$meta_data)
design2 <- cbind(design2, pData= study_arm =="ViTCV" & timepoint_3 =="V0")
design <- cbind(design2, Y= treatment=="Y" & timepoint=="T2")

#Just filter control samples for now
...
# Warning Coefficients not estimable: ViTCV
#Warning: Partial NA coefficients for 12969 probe(s)
